...
class Env:
    ...
    #     # compute relative wind velocity in world frame
    #     v_rel_wind = self.v - self.v_wind  # shape (B,3)

    #     # project into body axes (forward, left, up) using self.R columns
    #     # self.R is (B,3,3) with columns: forward=self.R[...,0], left=self.R[...,1], up=self.R[...,2]
    #     v_fwd_s = (v_rel_wind * self.R[..., 0]).sum(-1, keepdim=True)   # (B,1)
    #     v_left_s = (v_rel_wind * self.R[..., 1]).sum(-1, keepdim=True)  # (B,1)
    #     v_up_s = (v_rel_wind * self.R[..., 2]).sum(-1, keepdim=True)    # (B,1)

    #     # quadratic terms: s * abs(s)
    #     v_fwd_2 = v_fwd_s * v_fwd_s.abs()
    #     v_left_2 = v_left_s * v_left_s.abs()
    #     v_up_2 = v_up_s * v_up_s.abs()

    #     # Build per-axis drag contributions (a_drag_2 = quadratic-directional; a_drag_1 = linear-directional)
    #     # Note: z_drag_coef is (B,1) or None; treat it as scalar factor for up-component if present
    #     zcoef = self.z_drag_coef if self.z_drag_coef is not None else torch.zeros_like(v_up_s)

    #     a_drag_2 = v_up_2 * (self.R[..., 2] * zcoef) + v_left_2 * self.R[..., 1] + v_fwd_2 * self.R[..., 0]  # (B,3)
    #     a_drag_1 = v_up_s * (self.R[..., 2] * zcoef) + v_left_s * self.R[..., 1] + v_fwd_s * self.R[..., 0]      # (B,3)

    #     # drag_2 has shape (B,2): drag_2[:,0] multiplies the quadratic part, drag_2[:,1] multiplies the linear part
    #     drag = a_drag_2 * self.drag_2[:, :1] + a_drag_1 * self.drag_2[:, 1:2]   # broadcast to (B,3)
